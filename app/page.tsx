  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        if (mounted) {
          setModelError(
            "Failed to load the required models. Please check your connection and try again."
          );
          setModelLoading(false);
        }
      }
  ...
  return (
    <main className="container mx-auto py-10 px-4">
      <div className="flex justify-center mb-6">
        <Image
          src="/logo.png"
          alt="VISE by Intermarché"
          width={200}
          height={80}
          priority
        />
      </div>
      {/* <h1 className="text-3xl font-bold mb-6 text-center">VISE</h1> */}

      {modelLoading ? (
        <div className="flex flex-col items-center justify-center py-12">
          <Loader2 className="h-12 w-12 animate-spin text-primary mb-4" />
          <p className="text-muted-foreground">
            Loading image recognition models...
          </p>
        </div>
      ) : modelError ? (
        ...
        >
          <TabsList className="grid w-full grid-cols-3 mb-8">
            ...
            <TabsTrigger value="gallery">Images DB</TabsTrigger>
          </TabsList>
          ...
          {/* search pour les images page search */}
